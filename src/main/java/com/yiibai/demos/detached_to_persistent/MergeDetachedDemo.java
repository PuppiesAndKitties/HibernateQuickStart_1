...
/**
 * Hibernate 2版本有saveOrUpdateCopy(Object)方法。从Hibernate 3起，它被重命名为
 * merge(Object)。 因此与saveOrUpdate()相比，merge()方法有一些相似性和差异。
 * merge(Object)不会将对象置于Hibernate的管理下，而是创建一个对象的副本，而不是管理该对象。
 * 如果调用saveOrUpdate(aObject)则aObject由Hibernate管理，并且与aObject具有相同的ID将
 * 会抛出异常，但是使用merge(aObject)时不会得到此异常。
 */
